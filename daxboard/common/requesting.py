class RequestManager(object):
    ...
    def _is_valid(self):
        ...
        if (self._request.session.get('resource') and 
            ...
            self._request.GET.get('username') and 
    ...
    def raise_for_missing_argument(self):
        if self.is_login_attempt():
            if not self._is_valid():
                if not self._request.session.get('resource') and not self._request.GET.get('resource'):
                    raise ValueError('Argument RESOURCE has not been found on request.')
                elif not self._request.session.get('client_id') and not self._request.GET.get('client_id'):
                    raise ValueError('Argument CLIENT_ID has not been found on request.')
                elif not self._request.session.get('tenant') and not self._request.GET.get('tenant'):
                    raise ValueError('Argument TENANT has not been found on request.')
                else:
                    pass
                    # if not (self._request.session.get('client_secret') and self._request.GET.get('client_secret')) or not (self._request.session.get('username') or self._request.GET.get('username')) and not (self._request.session.get('password') or self._request.GET.get('password')):
                    #     raise ValueError('Please provide either CLIENT_SECRET or combination of USERNAME and PASSWORD arguments.')
                    # elif not (self._request.session.get('client_secret') and self._request.GET.get('client_secret')) and (self._request.session.get('username') or self._request.GET.get('username')) != (self._request.session.get('password') or self._request.GET.get('password')):
